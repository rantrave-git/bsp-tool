    ...
    for m in shader.get("pass", []):
        for p in m:
            ...
            if mp and not mp[0].startswith('$'):
                if update_callback is not None:
                    mp[0] = update_callback('texture', mp[0])
                yield mp[0]
            ...
            if mp and not mp[0].startswith('$'): 
                ...
                yield mp[0]
            mp = p.get('animmap', [])
            for ind, ani in enumerate(mp[1:]):
                if not ani.startswith('$'):
                    if update_callback is not None:
                        p['animap'][ind+1] = update_callback('texture', ani)
                    yield ani
