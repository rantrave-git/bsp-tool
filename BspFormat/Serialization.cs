...
public static class StreamExtensions
{
    ...
    public static async ValueTask<MeshContext?> TryReadMesh(this Stream stream, CancellationToken cancellationToken = default)
    {
    ...
    public static async ValueTask WriteArrayAsync<T>(this Stream stream, Memory<T> array) where T : struct
    {
    ...
    public static void WriteArray<T>(this Stream stream, Memory<T> array) where T : struct
    {
        stream.Write(array.Length);
        int step = MaxBufferSize / Marshal.SizeOf<T>();
        int i = 0;
    ...
    public static async ValueTask WriteMeshAsync(this Stream stream, MeshContext mesh)
    {
        await stream.WriteArrayAsync(mesh.Vertices.AsMemory());
        await stream.WriteArrayAsync(mesh.Corners.AsMemory());
        await stream.WriteArrayAsync(mesh.Faces.AsMemory());
    }
    public static void WriteMesh(this Stream stream, MeshContext mesh)
    {
        stream.WriteArray(mesh.Vertices.AsMemory());
        stream.WriteArray(mesh.Corners.AsMemory());
        stream.WriteArray(mesh.Faces.AsMemory());
    }
}
