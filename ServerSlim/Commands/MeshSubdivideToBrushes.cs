...
//     {
//         var mesh = await stream.TryReadMesh(cancellationToken);
//         if (mesh == null) throw new CommandHandlerException("Failed to read mesh");
//         // read content
//         var content = await stream.TryRead<long>(cancellationToken);
//         if (content == null) throw new CommandHandlerException("Failed to read mesh content");
//         return new Processor(mesh, content.Value);
//     }

// }

// public class MeshSubdivideToBrushesFactory : ICommandHandlerFactory
// {
//     public ICommandHandler CreateCommandHandler() => new MeshSubdivideToBrushes();
// }